...
class DenseExtractor(nn.Module):
    def __init__(self, nDenselayer=5, nFeat=128, growthRate=64):
    ...
    def forward(self, feature0, feature1,
        ...
        concat = torch.cat((feature0, feature1), dim=0)  # [2B, C, H, W]
        F_1 = self.RDB1(concat)
        F_2 = self.RDB2(F_1)
        F_3 = self.RDB3(F_2)
        FF = torch.cat((F_1, F_2, F_3), 1)
        FF = self.conv2(FF)
        feature0, feature1 = FF.chunk(chunks=2, dim=0)  # [B, C, H, W]
        # reshape back
...
class GMFlow(nn.Module):
    ...
    def forward(self, img0, img1,
        ...
        # resolution low to high， list = [1, 128, 40, 40]
